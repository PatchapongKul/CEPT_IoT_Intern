...
private:
    ...
    hvacSettings wantedSettings{"UNKNOWN", 0, "UNKNOWN", "UNKNOWN", "UNKNOWN", "UNKNOWN", "UNKNOWN", "UNKNOWN", "UNKNOWN", "UNKNOWN"}; // set data to prevent strcasecmp crash
    hvacSettings userSettings{"UNKNOWN", 0, "UNKNOWN", "UNKNOWN", "UNKNOWN", "UNKNOWN", "UNKNOWN", "UNKNOWN", "UNKNOWN", "UNKNOWN"};   // set data to prevent strcasecmp crash
    hvacStatus currentStatus{};

    // byte rxBuffer[250];
    // size_t rxBufferLength = 0;  // Track the number of valid bytes in rxBuffer
    // callback
    ...
    const byte FUNCTION_BYTE[17] = {128, 135, 136, 144, 148, 160, 163, 176, 179, 187, 190, 199, 222, 223, 247, 248, 203};
    const char *FUNCTION_BYTE_MAP[18] = {"STATE", "PSEL", "STATUS", "ONTIMER", "OFFTIMER", "FANMODE", "SWING", "MODE", "SETPOINT", "ROOMTEMP", "OUTSIDETEMP", "PURE", "WIFILED1", "WIFILED2", "OP", "FN_GROUP_1", "SELFCLEANING"
                                                                                                                                                                                                                  "UNKNOWN"};
    ...
    const char *SWING_BYTE_MAP[10] = {"fix", "vswing", "hswing", "hvswing", "fixpos_1", "fixpos_2", "fixpos_3", "fixpos_4", "fixpos_5", "UNKNOWN"};

    const byte SELFCLEAN_BYTE[2] = {24, 16};
    const char *SELFCLEAN_BYTE_MAP[3] = {"on", "off", "UNKNOWN"};

    const byte STATE_BYTE[3] = {49, 49, 48};
    const char *STATE_BYTE_MAP[4] = {"off", "off_selfcleaning", "on", "UNKNOWN"};
    const byte PURE_BYTE[2] = {16, 24};
    const byte TIMER_BYTE[2] = {66, 65};
    const byte WIFILED1_BYTE[2] = {0, 5};
    const byte WIFILED2_BYTE[6] = {128, 0, 4, 3, 2, 1};
    const char *WIFILED2_BYTE_MAP[7] = {"off", "on", "LED2_1Hz", "LED1_1Hz", "LED2_5Hz", "LED1_5Hz", "UNKNOWN"};
...
public:
    ...
    // void setSelfclean(const char *newSelfclean);
    ...
    const char *getSelfclean(void);
    bool isCduRunning(void);
    bool isConnected(void);
    void forceQueryAllData(void);
    bool isHandshaked(void);

    int8_t debug_selfcleaning = 0; // 0 - off, 1 - on

    bool sendCustomPacket(byte data[], size_t length);
    // const byte* getRxBuffer(void);
    // size_t getRxBufferLength(void);
};
